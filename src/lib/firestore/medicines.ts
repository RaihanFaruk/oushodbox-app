...
function mapDocToMedicine(docSnap: QueryDocumentSnapshot<DocumentData> | DocumentData, id: string): DatabaseMedicine & Record<string, any> {
  ...
  return {
    ...
    dosageBadge: data.dosageBadge || data.dosageForm || (data.itemType === "other" ? "সার্জিক্যাল" : "ট্যাবলেট"),
    ...
    unitPriceUnit: data.unitPriceUnit || (data.itemType === "other" ? "/পিস" : "/ট্যাবলেট"),
    ...
    dosageForm: data.dosageForm || (data.itemType === "other" ? "other" : "tablet"),
...
export function toAdminMedicineItem(doc: any): AdminMedicineItem {
  ...
  return {
    id: String(doc.id),
    itemType: doc.itemType || "medicine",
    tradeName: doc.tradeName || "Unknown Item",
    strength: doc.strength || "",
    dosageForm: doc.dosageForm || (doc.itemType === "other" ? "অন্যান্য" : "Tablet"),
